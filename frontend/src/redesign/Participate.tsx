 ...
 * sur `POST /submit` : le backend MASQUE le texte (PII), l'embedde (nomic local, aucun
 * LLM) et la corrèle INSTANTANÉMENT aux retours déjà reçus → on affiche un AGRÉGAT
 * non-PII « N personnes ont évoqué un sujet proche · X% du panel » (ou « parmi les
 * premiers ») + un remerciement. JAMAIS le verbatim d'un autre citoyen.
 */
          ...
          <section className="participate__result" aria-live="polite">
            {result.n_similar > 0 ? (
              <p className="participate__insight">
                ...
                {result.pct_panel > 0 && (
                  <> · <strong>{result.pct_panel}% du panel</strong></>
                )}
                .
              </p>
            ) : (
