...
*\* Mistral ms/avis = ~70s cumulés / 305 avis ≈ 230 ms/avis amorti (batché 12/appel, réseau UE) — cf. `llm_report.md`. ms/avis classifieur = embedding (dominant) + tête (quasi-nul), 100% sur le VPS. ms/avis Ollama = latence **À CHAUD** (warm-up préalable, modèle déjà chargé) cumulée / N, 1 avis/appel, sur le Mac de Bob (`http://macbat.tail0b8aa8.ts.net:11434`, Apple Silicon via Tailscale ; souverain — la donnée ne sort pas du réseau privé).*
