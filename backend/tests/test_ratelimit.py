...
frappe `POST /flag` avec un corps VIDE : la dépendance `rate_limit` s'exécute AVANT le
handler, donc sous le quota on obtient 422 (target_type manquant) et au-delà 429 —
sans aucun travail lourd. La fenêtre est vidée entre tests par l'autouse
`_reset_rate_limit`.
"""
...
PROTECTED_POST = "/flag"
BODY = {}                               # ni target_type ni avis_id → 422 tant qu'on passe le quota


def test_rate_limit_trips_429(client, monkeypatch):
    monkeypatch.setattr(auth, "_RATE", 3)
    auth._hits.clear()
    codes = [client.post(PROTECTED_POST, json=BODY).status_code for _ in range(4)]
    # Les 3 premières passent le quota (handler → 422 target_type manquant), la 4ᵉ est bloquée.
    assert codes[:3] == [422, 422, 422], codes
    assert codes[3] == 429, codes
    # Le corps 429 porte bien le message anti-abus.
    blocked = client.post(PROTECTED_POST, json=BODY)
...
def test_rate_limit_resets_after_clear(client, monkeypatch):
    ...
    auth._hits.clear()
    assert client.post(PROTECTED_POST, json=BODY).status_code == 422   # 1ʳᵉ passe
    assert client.post(PROTECTED_POST, json=BODY).status_code == 429   # 2ᵉ bloquée
    auth._hits.clear()                                                  # « le temps passe »
    assert client.post(PROTECTED_POST, json=BODY).status_code == 422   # ré-ouvert
